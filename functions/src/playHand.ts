...
export const playHandFunction = functions
  ...
  .onCall(async (data, context) => {
    ...
    const previousMove = moves.docs[0].data() as Card[] ?? [];
    ...
    if (moves.empty && (currentMove[0].suit !== 0 || currentMove[0].value !== 1)) {
      functions.logger.info(
    ...
    if (currentMove.length === 2) {
      ...
      functions.logger.info("playHand: playing doubles", { uid, gameId, isDouble });
    ...
    if (currentMove.length > 2) {
      ...
      functions.logger.info("playHand: playing triple or run", { uid, gameId, isTriple, isRun });
